...
| [日期](#日期-type5) | 5 | number \| string | `1675526400000` 或 `"2026-03-14"` | 毫秒时间戳或 `YYYY-MM-DD` 字符串 |
...
**写入格式**: Unix 毫秒时间戳，或 `YYYY-MM-DD` / `YYYY/MM/DD` 日期字符串（CLI 自动转换）
...
**⚠️ 重要**:
- 写入前必须先调用上传素材接口获取 `file_token`（使用 `feishu bitable upload-attachment`）
- 单个单元格附件数不超过 100
...
需在 `records` / `search` 命令加 `--automatic-fields` 才会返回。
...
**解决**: 先用 `feishu bitable upload-attachment` 上传到当前多维表格，使用返回的 `file_token` 写入记录。

---

### 字段名不存在 (错误码 1254045)

**排查**: 调用 `feishu bitable fields <app_token> <table_id>` 获取准确字段名，检查首尾空格。
...
// ✅ 正确
{ "参考链接": { "text": "示例网站", "link": "https://example.com" } }
```
...
一次性传入所有字段，避免多次调用。只传需要设置的字段，不必包含所有列。
